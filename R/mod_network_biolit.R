...
mod_network_biolit_ui <- function(id) {
  ...
  tagList(
    column(
      ...
      helpText("Si laissé vide, alors tous les animaux sont affichés (selon le nombre d'interactions minimum), cela permet de faire des comparaisons précises ou de regarder des réseaux plus simples"),
      ...
      helpText("Si laissé vide, alors toutes les algues sont affichées (selon le nombre d'interactions minimum), cela permet de faire des comparaisons précises ou de regarder des réseaux plus simples"),
      sliderInput(ns("max_interactions"),
                  "Nombre d'interactions représentées",
                  min = 2,
                  max = 3000,
                  value = 50),
      helpText("Ce champ permet de choisir le nombre d'interactions représentées. Cela permet de réduire la complexité du réseau mais biaise les données pour les taxons les plus abondants. Vous pouvez utiliser les filtres par espèces pour regarder les taxons plus rares."),
      checkboxInput(ns("normalise_interactions_algue"), label = "Normaliser les interactions selon les algues", value = TRUE),
      helpText("Si l'on utilise cette option, le nombre d'interactions est divisé par le nombre total d'interactions observées sur cette cinture alguale Attention certaines ceinture (comme le Lierre grimpant ou la carotte sauvage sont très représentés tandis que d'autres n'ont été vues que peut de fois) ce qui peut biaiser les résultats."),
      actionButton(
        ns("view_network"), 
